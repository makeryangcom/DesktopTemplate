        ...
        <div class="flex h-14 w-full items-center px-2">
            ...
            <div class="h-[36px] flex flex-1 items-center justify-end space-x-2">
                ...
                <div class="flex items-center no-drag">
                    ...
                    <Button @click="onBrowser('https://github.com/makeryangcom/DesktopTemplate')" class="w-8 h-8" variant="ghost" size="icon">
                ...
                <div class="flex items-center no-drag">
                    ...
                    <Button target="_blank" class="w-8 h-8" variant="ghost" size="icon" :title="!props.data.window.max ? '全屏' : '还原'" @click="onRightButton('size')">
                        <BoxIcon class="w-4 h-4" v-if="!props.data.window.max" />
                        <RotateCounterClockwiseIcon class="item-icon" v-if="props.data.window.max" />
                    </Button>
...
(window as any).base.ipc.on("message", (event: any, message: any) => {
    if(message.type === "main:window:resize"){
        props.data.window.max = message.data !== "restore";
    }
});
...
function onBrowser(url: string){
    (window as any).base.browser.openExternal(url);
}

function onRightButton(data: string){
    (window as any).base.ipc.send("message", {type: "template:header:right:button", data: data});
}
